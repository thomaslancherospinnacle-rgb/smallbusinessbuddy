/* ═══════════════════════════════════════════════════════════════
   FlowPipe Email Worker — Cloudflare Worker + Resend
   ═══════════════════════════════════════════════════════════════
   ...
   RESEND SETUP:
   1. resend.com → Add your domain (flowpipe.fit)
   ...
   1. improvmx.com → Add domain (flowpipe.fit)
   2. Add MX records in Cloudflare DNS
   3. Set forwarding: *@flowpipe.fit → yourgmail@gmail.com
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      const resendRes = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          from: from,          // e.g. "Support <support@flowpipe.fit>"
          to: [to],
