...
export default {
  async fetch(request, env) {
    ...
    if (request.method === 'GET' && path === '/') {
      return cors(json({ status: 'ok', worker: 'FlowPipe Stripe+Email', version: '2.0' }));
    }
...
async function handleCheckout(data, env) {
  ...
  const siteUrl = env.SITE_URL || 'https://flowpipe.fit';
  ...
  const params = new URLSearchParams({
    ...
    'payment_intent_data[description]': `FlowPipe deposit: ${service} on ${appointmentDate}`,
  });
...
async function handleInvoice(data, env) {
  ...
  const linkParams = new URLSearchParams({
    ...
    'after_completion[redirect][url]': `${env.SITE_URL || 'https://flowpipe.fit'}/booking-complete/index.html`,
  });
