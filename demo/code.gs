function testCalendar() {
  var cal = CalendarApp.getCalendarById('thomaslancheros06@gmail.com');
...
function testWorkerCall() {
  ...
  var response = UrlFetchApp.fetch(config.emailWorkerUrl, {
    ...
    payload: JSON.stringify({
      from: 'FlowPipe Plumbing <' + config.noreplyEmail + '>',
      to: 'thomaslancheros06@gmail.com',
...
/* ═══════════════════════════════════════════════════════════════
   FlowPipe — Google Apps Script Backend  (v3)
   ═══════════════════════════════════════════════════════════════
...
/**
 * Parse a date string like "Mar 15, 2026" and time like "2:30 PM"
...
function sendAutoReply(type, data) {
  ...
  var config = getConfig();
  var bizName = config.businessName || 'FlowPipe Plumbing';
  ...
  // Format: "FlowPipe Plumbing <noreply@flowpipe.fit>"
...
function getConfig() {
  ...
  if (!sheet) {
    ...
    const defaults = [
      ['businessName', 'FlowPipe Plumbing'],
      ['businessPhone', '(954) 555-0199'],
      ['businessEmail', 'info@flowpipe.com'],
...
function _seedDefaultServices(sheet) {
  var defaults = [
    [generateId(), 1, 'Emergency Repair',  '🔧', '24/7 emergency plumbing repairs for burst pipes, major leaks, and flooding.',        'From $89',  60,  true],
    ...
    [generateId(), 6, 'Inspection',        '🔍', 'Comprehensive plumbing inspection with camera and detailed report.',                 'From $49',  45,  true],
...
function _seedDefaultAbout(sheet) {
  var defaults = [
    ['headline',         'Trusted Plumbing, Done Right'],
    ['subheadline',      'Family-owned and serving South Florida since 2006'],
    ['story',            'FlowPipe started as a one-truck operation out of Fort Lauderdale. Over 18 years we have grown into a team of licensed master plumbers who treat every home like their own. We believe in honest pricing, showing up on time, and never leaving a job half done.'],
    ['mission',          'To deliver reliable, fairly priced plumbing services with the transparency and respect every homeowner deserves.'],
    ['yearsInBusiness',  '18+'],
    ['jobsCompleted',    '4,500+'],
    ['satisfactionRate', '99%'],
    ['teamSize',         '12'],
